...
int main(int argc, const char *argv[]) {
    ...
    std::vector<double> lidar_TTCs;
    std::vector<double> camera_TTCs;
        ...
        if (dataBuffer.size() > 1)  // wait until at least two images have been processed
        {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1) {
                ...
                if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0)  // only compute TTC if we have Lidar points
                {
                    ...
                    camera_TTCs.push_back(ttcCamera);
                    //// EOF STUDENT ASSIGNMENT
    ...
    std::cout << "Lidar TTCs:" << std::endl;
    for (const auto ttc : lidar_TTCs) {
        std::cout << ttc << std::endl;
    }
    std::cout << "Camera TTCs:" << std::endl;
    for (const auto ttc : camera_TTCs) {
        std::cout << ttc << std::endl;
    }
